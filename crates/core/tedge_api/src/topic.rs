use crate::{error::TopicError, build_topic};
...
impl ResponseTopic {
    pub fn as_str(&self) -> String {
        match self {
            Self::SoftwareListResponse => build_topic("commands/res/software/list"),
            Self::SoftwareUpdateResponse => build_topic("commands/res/software/update"),
            Self::RestartResponse => build_topic("commands/res/control/restart"),
        }
    }
}
...
impl TryFrom<String> for ResponseTopic {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        ...
        if topic == build_topic("commands/res/software/list") {
            Ok(ResponseTopic::SoftwareListResponse)
        } else if topic == build_topic("commands/res/software/update") {
            Ok(ResponseTopic::SoftwareUpdateResponse)
        } else if topic == build_topic("commands/res/control/restart") {
            Ok(ResponseTopic::RestartResponse)
        } else {
...
impl RequestTopic {
    pub fn as_str(&self) -> String {
        match self {
            Self::SoftwareListRequest => build_topic("commands/req/software/list"),
            Self::SoftwareUpdateRequest => build_topic("commands/req/software/update"),
            Self::RestartRequest => build_topic("commands/req/control/restart"),
        }
    }
}
...
mod tests {
    ...
    fn convert_response_topic_to_str() {
        assert_eq!(
            ...
            build_topic("commands/res/software/list")
        );
        assert_eq!(
    ...
    fn convert_str_into_response_topic() {
        let list: ResponseTopic = build_topic("commands/res/software/list").try_into().unwrap();
        assert_eq!(list, ResponseTopic::SoftwareListResponse);
        let update: ResponseTopic = build_topic("commands/res/software/update").try_into().unwrap();
