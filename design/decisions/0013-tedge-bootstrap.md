...
cloud (with an optional derived `type`),
methods (with implied `set` values), env-var inputs, settings,
...
The `[url]` block and a `[[settings]]` entry additionally support `prompt = false`,
which requires a default:
the wizard states the value instead of asking for it,
and non-interactive runs apply it too
(an explicit `--url` or `--set` still wins).
This is how a vendor package locks a cloud to one endpoint,
or a derived cloud pins a config value such as its transport,
with one vocabulary for the URL and the settings
rather than a separate cloud-level `set` table.
Otherwise the defaults have different scope:
...
* **Connect**: custom mappers do not use the mosquitto bridge;
  the mapper connects to the cloud itself,
  with the built-in bridge of the generic mapper or one of its own.
  ...
  The wait has two signals:
  the mapper's own health status, always,
  and the `tedge-mapper-bridge-<name>` sub-service's health status
  when the mapper directory has a `bridge/` sub-directory -
  the latter is the actual proof of a cloud connection.
  A mapper that is its own bridge provides only the first signal,
  so the contract is that it reports `up` only once connected to the cloud;
...
* Inheritance is per-section, favoring the derived descriptor:
  ...
  `[[settings]]` are always the derived cloud's own,
...
* A `[[settings]]` entry declared `prompt = false` pins a config value
