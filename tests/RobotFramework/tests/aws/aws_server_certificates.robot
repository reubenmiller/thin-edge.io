...
Connect using certificates generated by AWS
    ...
    # Check twin data (simple)
    ${cloud_topic}=    AWS.Get Cloud Telemetry Topic
    ...    common_name=${DEVICE_SN}
    ...    te_topic=te/device/main///twin/foo
    Execute Command    tedge mqtt pub -r te/device/main///twin/foo '"bar"'
    ${messages}=    AWS.Should Have MQTT Messages    topic=${cloud_topic}    min_count=1
    Should Not Be Empty    ${messages}

    # Check twin data (complex)
    ${cloud_topic}=    AWS.Get Cloud Telemetry Topic
    ...    common_name=${DEVICE_SN}
    ...    te_topic=te/device/main///twin/complex
    Execute Command    tedge mqtt pub -r te/device/main///twin/complex '{"name":"other"}'
    ${messages}=    AWS.Should Have MQTT Messages    topic=${cloud_topic}    min_count=1
    Should Not Be Empty    ${messages}
