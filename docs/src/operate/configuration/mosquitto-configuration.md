...
By default, `tedge connect` and `tedge refresh-bridges` create the file
...
With this setting, none of the %%te%% commands
(`tedge connect`, `tedge reconnect` and `tedge refresh-bridges`)
write `tedge-mosquitto.conf`,
