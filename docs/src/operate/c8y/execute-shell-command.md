...
A command which has not completed after one hour is terminated, along with the processes it has started,
and the operation fails with the reason `Command timed out`.
...
The `tedge` package upgrade restarts the mapper but not the agent,
so restart the agent once the community package is gone.
