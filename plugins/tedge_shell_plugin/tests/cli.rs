...
fn the_shell_can_be_overridden_from_the_command_line() {
    ...
    plugin(&config_dir)
        ...
        .stdout(contains(
            r#""reason":"Failed to run the command: cannot launch the shell '/no/such/shell'"#,
        ))
        .stderr(contains("/no/such/shell"));
}

#[test]
fn an_empty_command_is_reported_as_such() {
    let config_dir = TempTedgeDir::new();
    plugin(&config_dir)
        .args(["--command", "  "])
        .assert()
        .failure()
        .stdout(contains(r#""reason":"No command to execute""#));
}
...
fn a_missing_tmp_dir_is_reported_as_such() {
    ...
    config_dir
        .file("tedge.toml")
        .with_raw_content("[tmp]\npath = \"/no/such/dir\"\n");

    plugin(&config_dir)
        .args(["--command", "echo hello"])
        .assert()
        .failure()
        // The reason names the tmp dir rather than the shell, as the shell is fine
        .stdout(contains(
            r#""reason":"Failed to run the command: cannot create a temporary output file in the tmp dir '/no/such/dir'"#,
        ))
