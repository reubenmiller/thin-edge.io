...
///
/// Each variant names the path which could not be used,
/// so the user is told which configuration setting to fix.
#[derive(Debug, thiserror::Error)]
pub enum ShellError {
    #[error("cannot create a temporary output file in the tmp dir '{tmp_dir}': {source}")]
    TmpDir {
        tmp_dir: Utf8PathBuf,
        #[source]
        source: std::io::Error,
    ...
    #[error("cannot launch the shell '{shell}': {source}")]
    Shell {
        shell: Utf8PathBuf,
        #[source]
        source: std::io::Error,
    ...
    #[error("cannot read the command output: {0}")]
...
pub fn execute(
    ...
    let mut output_file = tempfile::tempfile_in(tmp_dir).map_err(|source| ShellError::TmpDir {
    ...
    let status = Command::new(shell)
        ...
        .status()
        .map_err(|source| ShellError::Shell {
...
mod tests {
    ...
    fn execute_with_defaults(command: &str) -> Result<ShellOutcome, ShellError> {
    ...
    #[test]
    fn a_missing_shell_is_reported_as_such() {
        let err = execute(
        ...
        .unwrap_err();
        let ShellError::Shell { shell, source } = &err else {
        ...
        assert_eq!(
            err.to_string(),
            "cannot launch the shell '/no/such/shell': No such file or directory (os error 2)"
        );
    }

    #[test]
    fn a_missing_tmp_dir_is_reported_as_such() {
        let err = execute(sh(), "echo hello", Utf8Path::new("/no/such/dir"), NO_LIMIT).unwrap_err();
        let ShellError::TmpDir { tmp_dir, source } = &err else {
            panic!("unexpected error: {err}");
        };
        assert_eq!(tmp_dir, "/no/such/dir");
        assert_eq!(source.kind(), std::io::ErrorKind::NotFound);
        assert_eq!(
            err.to_string(),
            "cannot create a temporary output file in the tmp dir '/no/such/dir': No such file or directory (os error 2)"
        );
    }
