...
impl ManagedDir {
    ...
    pub fn relative_path_from_root(&self) -> &Utf8Path {
        self.path
    ...
    pub fn template_file(
        ...
        let relative_from_root = self.relative_path_from_root().join(path);
        TedgePaths {
            root: self.root.clone(),
...
mod tests {
    ...
    async fn template_persistence_does_not_restore_a_deleted_active_file() {
        ...
        let root = TedgePaths::from_root_with_defaults(ttd.path(), "", "");
